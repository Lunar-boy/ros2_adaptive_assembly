#!/usr/bin/env bash

set -euo pipefail

topic_name="/panda_pre_grasp_pose"
expected_type="geometry_msgs/msg/PoseStamped"
expected_frame="panda_link0"

if ! topic_info="$(ros2 topic info "${topic_name}" 2>/dev/null)"; then
  echo "FAIL: ${topic_name} does not exist or ROS 2 is not available"
  exit 1
fi

if ! grep -q "^Type: ${expected_type}$" <<< "${topic_info}"; then
  echo "FAIL: ${topic_name} has unexpected type"
  echo "      expected: ${expected_type}"
  echo "${topic_info}" | sed 's/^/      /'
  exit 1
fi

if ! message="$(ros2 topic echo --once "${topic_name}" 2>/dev/null)"; then
  echo "FAIL: could not read one message from ${topic_name}"
  exit 1
fi

if ! grep -q "frame_id: ${expected_frame}" <<< "${message}"; then
  echo "FAIL: ${topic_name} does not use frame_id ${expected_frame}"
  echo "${message}" | sed 's/^/      /'
  exit 1
fi

echo "PASS: ${topic_name} exists with type ${expected_type} and frame_id ${expected_frame}"
