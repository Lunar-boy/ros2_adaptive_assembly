...
check_package() {
  ...
  case "${package_name}" in
    moveit_ros_planning_interface|moveit_resources_panda_moveit_config)
...
main() {
  ...
  check_package "moveit_ros_planning_interface" || failures=$((failures + 1))
  ...
  echo "PASS: all static PlanningScene packages are available"
}
