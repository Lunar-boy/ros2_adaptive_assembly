...
public:
  ...
  : node_(node),
    ...
    status_topic_(declare_parameter<std::string>("status_topic", "/panda_pose_adapter_status")),
    ...
    normalize_quaternion_(declare_parameter<bool>("normalize_quaternion", true))
  {
    ...
    adapted_pose_publisher_ =
      ...
      output_topic_, 10);
    status_publisher_ =
      node_->create_publisher<std_msgs::msg::String>(status_topic_, 10);
...
private:
  ...
  void pre_grasp_pose_callback(const geometry_msgs::msg::PoseStamped & input_pose)
  {
    ...
    if (input_pose.header.frame_id.empty()) {
      ...
      RCLCPP_WARN(
        node_->get_logger(),
        "Skipping Panda pose adaptation because input frame_id is empty.");
      return;
    }
    ...
    if (use_tf_transform_) {
      try {
        ...
        RCLCPP_WARN(
          node_->get_logger(),
          "Failed to transform pre-grasp pose from frame '%s' to '%s': %s",
          input_pose.header.frame_id.c_str(), target_frame_id_.c_str(),
          exception.what());
        return;
      }
    } else {
