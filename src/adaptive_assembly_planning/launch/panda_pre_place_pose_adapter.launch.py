"""Adapt task-level pre place poses into the Panda planning frame."""
...
def generate_launch_description() -> LaunchDescription:
    """Start the parameterized Panda pose adapter for pre place poses."""
    return LaunchDescription([
        ...
        Node(
            ...
            name='panda_pre_place_pose_adapter_node',
            output='screen',
            parameters=[{
                'use_sim_time': ParameterValue(
                    LaunchConfiguration('use_sim_time'), value_type=bool
                ),
                'input_topic': '/pre_place_pose',
                'output_topic': '/panda_pre_place_pose',
                'output_frame_id': 'panda_link0',
                'use_tf_transform': False,
                'target_frame_id': 'panda_link0',
                'tf_lookup_timeout_sec': 0.2,
                'status_topic': '/panda_pre_place_pose_adapter_status',
                'x_offset': 0.0,
                'y_offset': 0.0,
                'z_offset': 0.0,
