"""Launch the deterministic wide Panda planning benchmark profile."""
...
def generate_launch_description() -> LaunchDescription:
    """Start the Panda planning demo with wide benchmark parameters."""
    ...
    benchmark_params_file = PathJoinSubstitution([
        FindPackageShare('adaptive_assembly_bringup'),
        'config',
        'adaptive_assembly_benchmark_wide_params.yaml',
    ])

    return LaunchDescription([
        LogInfo(
            msg='Launching wide deterministic Panda planning benchmark: '
            'wide target distribution with random_seed=202. Trajectories '
            'are not executed.'
        ),
