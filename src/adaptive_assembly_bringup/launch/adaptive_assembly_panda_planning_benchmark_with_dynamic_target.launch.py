"""Launch the seeded Panda planning benchmark with dynamic target object."""
...
def generate_launch_description() -> LaunchDescription:
    """Start seeded planning benchmark with dynamic target scene enabled."""
    ...
    return LaunchDescription([
        LogInfo(
            msg='Launching seeded Panda planning benchmark with the dynamic '
            'target collision object. Trajectories are not executed.'
        ),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(panda_planning_demo_launch),
            launch_arguments={
                'params_file': benchmark_params_file,
                'use_dynamic_target_scene': 'true',
            }.items(),
        ),
    ])
